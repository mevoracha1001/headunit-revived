...
import com.adamate.aaforaaos.aap.protocol.messages.NightModeEvent
import com.adamate.aaforaaos.connection.CommManager
import com.adamate.aaforaaos.connection.JmdnsServiceAdvertiser
import com.adamate.aaforaaos.connection.NetworkDiscovery
import com.adamate.aaforaaos.connection.WifiDirectManager
...
import com.adamate.aaforaaos.connection.UsbReceiver
...
import com.adamate.aaforaaos.utils.LogExporter
import com.adamate.aaforaaos.utils.NightModeManager
...
class AapService : Service(), UsbReceiver.Listener {
    ...
    /**
     * Enables car mode for phone/tablet head units.
    ...
    private fun scheduleReconnectIfNeeded(state: CommManager.ConnectionState.Disconnected) {
        ...
        if (lastType == com.adamate.aaforaaos.utils.Settings.CONNECTION_TYPE_USB &&
        ...
        if (!state.isClean) {
            ...
            if (mode == 1 && lastType != com.adamate.aaforaaos.utils.Settings.CONNECTION_TYPE_USB) {
    ...
    /** Acquires MulticastLock for JmDNS (in-app mDNS). Required for multicast to work. */
    private fun acquireMulticastLock() {
        if (multicastLock == null) {
            val wifiManager = applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager
            multicastLock = wifiManager.createMulticastLock("HeadunitRevived:JmDNS").apply {
                setReferenceCounted(false)
            }
        }
        if (multicastLock?.isHeld != true) {
            multicastLock?.acquire()
            AppLog.i("MulticastLock acquired for JmDNS")
        }
    }
    ...
    private fun releaseMulticastLock() {
        if (multicastLock?.isHeld == true) {
            ...
            AppLog.i("MulticastLock released")
        }
    }
    ...
    private fun createNotification(): Notification {
        ...
        val (notificationIntent, requestCode) = if (commManager.isConnected) {
            ...
            Intent(this, com.adamate.aaforaaos.main.MainActivity::class.java).apply {
    ...
    /**
     * Called when Android Auto app is not available (no GMS, restricted device, etc).
     * Server is already running on 127.0.0.1:5288. Copy connection info to clipboard
     * and inform user how to connect from another device.
     */
    private fun onSelfModeNoAndroidAuto() {
        val connInfo = "127.0.0.1:5288"
        try {
            val clipboard = getSystemService(Context.CLIPBOARD_SERVICE) as android.content.ClipboardManager
            clipboard.setPrimaryClip(android.content.ClipData.newPlainText("Headunit", connInfo))
        } catch (e: Exception) {
            AppLog.w("Could not copy to clipboard", e)
        }
        Toast.makeText(
            this,
            getString(R.string.self_mode_no_aa_fallback, connInfo),
            Toast.LENGTH_LONG
        ).show()
    }
     ...
     *
     * Registers the service over mDNS using JmDNS (in-app, no GMS/system dependency) as
     * `_aawireless._tcp` so Android Auto Wireless clients can discover it. Each accepted
     * socket is handed off to [CommManager.connect]. Only one connection at a time.
     *
    ...
    private inner class WirelessServer {
        ...
        private var jmdnsAdvertiser: JmdnsServiceAdvertiser? = null
        private var job: Job? = null

        fun start() {
            acquireMulticastLock()
            jmdnsAdvertiser = JmdnsServiceAdvertiser(this@AapService)
            if (jmdnsAdvertiser?.register(5288) != true) {
                AppLog.w("WirelessServer: JmDNS registration failed; discovery may not work.")
            }
            ...
            job = serviceScope.launch(Dispatchers.IO) {
                try {
                    ...
                    jmdnsAdvertiser?.unregister()
                    jmdnsAdvertiser = null
    ...
    companion object {
        ...
        const val ACTION_START_SELF_MODE           = "com.adamate.aaforaaos.ACTION_START_SELF_MODE"
        const val ACTION_START_WIRELESS            = "com.adamate.aaforaaos.ACTION_START_WIRELESS"
        const val ACTION_START_WIRELESS_SCAN       = "com.adamate.aaforaaos.ACTION_START_WIRELESS_SCAN"
        const val ACTION_STOP_WIRELESS             = "com.adamate.aaforaaos.ACTION_STOP_WIRELESS"
        const val ACTION_CHECK_USB                 = "com.adamate.aaforaaos.ACTION_CHECK_USB"
        const val ACTION_STOP_SERVICE              = "com.adamate.aaforaaos.ACTION_STOP_SERVICE"
        const val ACTION_DISCONNECT                = "com.adamate.aaforaaos.ACTION_DISCONNECT"
        const val ACTION_REQUEST_NIGHT_MODE_UPDATE = "com.adamate.aaforaaos.ACTION_REQUEST_NIGHT_MODE_UPDATE"
        const val ACTION_NIGHT_MODE_CHANGED      = "com.adamate.aaforaaos.ACTION_NIGHT_MODE_CHANGED"
        /**
         ...
         */
        const val ACTION_CONNECT_SOCKET            = "com.adamate.aaforaaos.ACTION_CONNECT_SOCKET"
