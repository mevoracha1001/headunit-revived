...
- **USB: Projection crashes during "Android Auto is loading":** If the projection briefly shows "loading" then returns to the main screen, the USB handshake may be failing (common with some car USB ports). **Fixes:** (1) Try a different USB cable—data-capable cables work best. (2) Use a different USB port in the car if available. (3) The app waits for auto-reconnect (7s on GM/AAOS cars, 3.5s elsewhere)—wait for "Retrying…" to complete. (4) Enable **Settings → Debug → Log capture** and reproduce the issue, then share logs to diagnose handshake/SSL errors.
- **GM Android Automotive (AAOS) USB:** The app is optimized for GM cars with Android Automotive. It uses extended USB timeouts, software-first video decoding with hardware fallback, 720p/480p resolution caps, and longer reconnect grace periods. If projection still crashes, ensure you're using a high-quality data-capable USB cable.
